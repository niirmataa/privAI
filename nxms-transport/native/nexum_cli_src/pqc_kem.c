...
static int ff_kem_is_frodokem_640_shake(const char *alg) {
    return alg && strcmp(alg, OQS_KEM_alg_frodokem_640_shake) == 0;
}
...
int ff_kem_keygen(const char *alg, ff_kem_keys_t *out) {
    ...
    if (ff_kem_is_frodokem_640_shake(alg)) {
        out->pk_len = OQS_KEM_frodokem_640_shake_length_public_key;
        out->sk_len = OQS_KEM_frodokem_640_shake_length_secret_key;
...
#endif
        OQS_STATUS keypair_rc = force_keypair_fail
            ? OQS_ERROR
            : OQS_KEM_frodokem_640_shake_keypair(out->pk, out->sk);
        if (keypair_rc != OQS_SUCCESS) {
            free(out->pk);
            out->pk = NULL;
            out->pk_len = 0;
            kem_secret_free(&out->sk, &out->sk_len);
...
#endif
    OQS_STATUS keypair_rc = force_keypair_fail ? OQS_ERROR : OQS_KEM_keypair(kem, out->pk, out->sk);
    if (keypair_rc != OQS_SUCCESS) {
        OQS_KEM_free(kem);
        free(out->pk);
        out->pk = NULL;
...
int ff_kem_encaps(const char *alg, const uint8_t *pk, size_t pk_len,
    ...
    *ct = NULL; *ss = NULL; *ct_len = 0; *ss_len = 0;
    ...
    if (ff_kem_is_frodokem_640_shake(alg)) {
        if (!pk || pk_len != OQS_KEM_frodokem_640_shake_length_public_key) return -1;

        *ct_len = OQS_KEM_frodokem_640_shake_length_ciphertext;
        *ss_len = OQS_KEM_frodokem_640_shake_length_shared_secret;
...
#endif
        if (force_encaps_fail || OQS_KEM_frodokem_640_shake_encaps(*ct, *ss, pk) != OQS_SUCCESS) {
            free(*ct);
            *ct = NULL;
            *ct_len = 0;
            kem_secret_free(ss, ss_len);
...
int ff_kem_decaps(const char *alg, const uint8_t *sk, size_t sk_len,
    ...
    *ss = NULL; *ss_len = 0;
    ...
    if (ff_kem_is_frodokem_640_shake(alg)) {
        if (!sk || sk_len != OQS_KEM_frodokem_640_shake_length_secret_key) return -1;
        if (!ct || ct_len != OQS_KEM_frodokem_640_shake_length_ciphertext) return -1;
...
#endif
        if (force_decaps_fail || OQS_KEM_frodokem_640_shake_decaps(*ss, ct, sk) != OQS_SUCCESS) {
            kem_secret_free(ss, ss_len);
            return -1;
        }
