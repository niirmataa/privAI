...
static void derive_seed(
    ...
    shake256_context sc;
    uint8_t variant_le[4];

    variant_le[0] = (uint8_t)variant_index;
    variant_le[1] = (uint8_t)(variant_index >> 8);
    variant_le[2] = (uint8_t)(variant_index >> 16);
    variant_le[3] = (uint8_t)(variant_index >> 24);

    shake256_init(&sc);
    shake256_inject(&sc, "NXMS-FALCON-FUZZ-SEED", 21);
    shake256_inject(&sc, variant_le, sizeof variant_le);
    if (program && program_len) {
        shake256_inject(&sc, program, program_len);
    }
    shake256_flip(&sc);
    shake256_extract(&sc, seed, 48);
}

static int falcon_sign_ct_deterministic(
    const uint8_t *sk,
    size_t sk_len,
    const uint8_t *msg,
    size_t msg_len,
    ...
    logn = falcon_get_logn((void *)sk, sk_len);
    if (logn != FF_FALCON_LOGN) {
        return -1;
    }

    shake256_init_prng_from_seed(&rng, seed, 48);
    tmpsz = FALCON_TMPSIZE_SIGNDYN(FF_FALCON_LOGN);
    tmp = malloc(tmpsz);
    if (!tmp) {
        return -1;
    }
...
static int falcon_verify_ct_direct(
    const uint8_t *pk,
    size_t pk_len,
    const uint8_t *msg,
    size_t msg_len,
    ...
    logn = falcon_get_logn((void *)pk, pk_len);
    if (logn != FF_FALCON_LOGN) {
        return -1;
    }

    tmpsz = FALCON_TMPSIZE_VERIFY(FF_FALCON_LOGN);
    tmp = malloc(tmpsz);
    if (!tmp) {
        return -1;
    }
    ...
    rc = falcon_verify(sig, sig_len, FALCON_SIG_CT, pk, pk_len, msg, msg_len, tmp, tmpsz);
    free(tmp);
    return (rc == 0) ? 0 : -1;
}
...
static int run_case(const fixture_t *fixture, const uint8_t *program, size_t program_len, int expect_success) {
    ...
    variant_index = (program_len > 0) ? ((size_t)program[0] % fixture->message_count) : 0u;
    ...
    derive_seed(variant_index, program, program_len, seed);
    rc = falcon_sign_ct_deterministic(
    ...
    rc = falcon_verify_ct_direct(pk, fixture->sender_pk_sig_len, msg, msg_len, sig, sig_len);
