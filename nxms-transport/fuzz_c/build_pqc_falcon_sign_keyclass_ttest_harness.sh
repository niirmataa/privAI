...
OUT="$ROOT/fuzz_c/fuzz_pqc_falcon_sign_keyclass_ttest"
CC_BIN="${CC:-cc}"
CFLAGS_EXTRA="${CFLAGS:-}"
LDFLAGS_EXTRA="${LDFLAGS:-}"

$CC_BIN -std=c11 -O3 $CFLAGS_EXTRA \
  -DFF_FALCON_LOGN=10 \
  -DNXMS_FALCON_AUDIT_RAW_API=1 \
  -DFALCON_FPEMU=1 \
  -DFALCON_FPNATIVE=0 \
  -DFALCON_UNALIGNED=0 \
  -I"$ROOT/native" \
  ...
  "$ROOT/fuzz_c/pqc_falcon_sign_keyclass_ttest_harness.c" \
