...
use nxms_transport::crypto::{
    ...
    decrypt_for_context, encrypt, encrypt_for_context, encrypt_for_context_with_signer, kem_decaps,
...
use nxms_transport::wire::{
    ESCROW_APP_PROTO_V1, MsgType, NXMS_PROTO_V1, NXMS_PROTO_V2, NxmsEnvelope, NxmsEnvelopeV2,
    NxmsPayload, NxmsPayloadV2, msg_type_key,
};
use sha3::{
    Shake256,
...
#[test]
fn crypto_roundtrip_encrypt_decrypt_legacy_escrow() {
...
#[test]
fn crypto_roundtrip_encrypt_decrypt_v2_context() {
    let sender = Keys::generate().expect("sender keys");
    let recipient = Keys::generate().expect("recipient keys");
    ...
    let payload = NxmsPayloadV2 {
        ...
        to: "bob".to_string(),
        seq,
        data: "{\"offer\":\"local-model\"}".to_string(),
    };
    let plaintext = serde_json::to_vec(&payload).expect("payload json");
